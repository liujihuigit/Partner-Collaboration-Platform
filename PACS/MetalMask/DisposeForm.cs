...
namespace MetalMask
{
    public partial class DisposeForm : XtraForm
    {
        private string receivedata = "";
        private delegate void InvokeDelegate(string data);
        private DataTable dt = new DataTable();
        public DisposeForm(List<string> barcodeList)
        {
            InitializeComponent();
            DataColumn dc = null;
            ...
            MetalMaskGlobal.port.DataReceived += new SerialDataReceivedEventHandler(serialPortDispose_DataReceived);
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < gridView1.RowCount; i++)
                {
                    ...
                    {
                        XtraMessageBox.Show("请选择 Reason", "提示");
                        return;
                    }
                    string ReasonValue = ReasonText.Split(':')[1];

                    sql = "update pacsm_rm_tool set DSU_RSN_CODE = '" + ReasonValue + "',status_code = 'MBDSU',update_dt = to_char(sysdate,'yyyyMMddhh24miss'),update_user ='" + PaCSGlobal.LoginUserInfo.Id + "' "+
                        " where  tool_gubun_code ='MM' "+
                        " and fct_code = '" + PaCSGlobal.LoginUserInfo.Fct_code + "' " +
        ...
        private void DisposeForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            MetalMaskGlobal.port.DataReceived += new SerialDataReceivedEventHandler(serialPortDispose_DataReceived);
        }
    }
