...
{
    /// <summary>
    /// gridview 增加Check Box 
    /// 使用方法：
    /// GridCheckMarksSelection grdCheckSelect = new GridCheckMarksSelection(grdSN);   // gridView 增加Check Box 
    /// grdCheckSelect.CheckMarkColumn.VisibleIndex = N; //第几列追加（N = INDEX 0 . 1 ）
    /// 
    /// 选中事件：SelectionChanged
    /// private void grdCheckSelect_SelectionChanged(object sender, EventArgs e)
    /// { 选中事件 }
    /// 单选：MultiSelect Mode =  CheckBoxRowSelect
    /// </summary>
    public class GridCheckMarksSelection
    {
        ...
        protected virtual void Attach()
        {
            ...
            _column.OptionsColumn.ShowCaption = false;   //不显示标题栏
